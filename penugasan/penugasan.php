...
<script type="text/javascript"> 
function PRINT(pk){ 
win=window.open('penugasan/penugasan_print.php?pk='+pk,'win','width=1000, height=400, menubar=0, scrollbars=1, resizable=0, location=0, toolbar=0, status=0'); } 
...
<tr>
<td height="24"><label for="id_rekanan">Pilih Rekanan</label>
...
<td colspan="2"><input name="Simpan" type="submit" class="btn btn-primary"  id="Simpan" value="Simpan" />
        ...
        <a href="?mnu=penugasan"><input name="Batal" class="btn btn-danger" type="button" id="Batal" value="Batal" /></a>
</td></tr>
</table>
</form>
<br />
...
<?php  
  $sqlc="select distinct(`id_rekanan`) from `$tbpenugasan` order by `status` asc";
  $jumc=getJum($conn,$sqlc);
	...
	$arrc=getData($conn,$sqlc);
		foreach($arrc as $dc) {						
				...
				$sql = "select * from `$tbpenugasan` where  `id_rekanan`='$id_rekanan' order by `id_penugasan` desc";
		$jum = getJum($conn, $sql);
	?>
		<h3>Data Penugasan <?php echo "$rekanan |$id_rekanan ($jum Data)"?>:</h3>
<div>				
...
echo"<tr bgcolor='$color'>
				...
				<td><small>$status</td>
				<td><div align='center'>
					<a href='?mnu=penugasan_detail&id=$id_penugasan'><img src='ypathicon/xls.png' title='Detail Penugasan'></a> 
				<br>
				<a href='?mnu=penugasan&pro=ubah&kode=$id_penugasan'><img src='ypathicon/ub.png' title='ubah'></a>
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=penugasan'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=penugasan'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=penugasan'>Next »</a></span>";
	}
...
$simpan=process($conn,$sql);
	if($simpan === TRUE) {
	echo"<script>berhasil('penugasan','Sukses Simpan');</script>";}
		else{
			echo"<script>gagal('penugasan','Gagal Simpan');</script>";}
	}
	else{
	...
	$ubah=process($conn,$sql);
		if($ubah) {echo "<script>berhasil('penugasan','Sukses Ubah');</script>";}
		else{echo"<script>gagal('penugasan','Gagal Ubah');</script>";}
	}//else simpan
}
?>
