...
<?php  
  $sqlc="select distinct(`$tbabsensi`.`id_finger`) from `$tbabsensi` where `$tbabsensi` .`id_mesin`='$id_mesin' order by `$tbabsensi`.`id_finger` asc";
  $jumc=getJum($conn,$sqlc);
	...
	$gabs="| ";
		...
		$gabs.="<a href='?mnu=rabsensi&idf=$id_finger'>$nama_karyawan</a> | ";			
	}
echo $gabs;
if(isset($_GET["idf"])){
	$id_finger=$_GET["idf"];
...
| <img src='ypathicon/print.png' title='PRINT' OnClick="PRINT('<?php echo $id_finger;?>')"> |
<br>
				...
				$knorm="$lamakerja1 Jam Tidak Mencukupi";
					if(floor($lamakerja1)>0){
						if($kategori=="Weekday"){
							...
							$lamakerja1=$lamakerja1-9;//9 jam acuan kerja maksimal
...
if($jmldata>0){
	...
	if($page > 1){
		$prev=$page-1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$prev&mnu=rabsensi'>« Prev</a></span> ";
	}
	else{echo "<span class=disabled>« Prev</span> ";}

	for($i=1;$i<=$jmlhal;$i++)
	if ($i != $page){echo "<a href='$_SERVER[PHP_SELF]?page=$i&mnu=rabsensi'>$i</a> ";}
	else{echo " <span class=current>$i</span> ";}

	if($page < $jmlhal){
		$next=$page+1;
		echo "<span class=prevnext><a href='$_SERVER[PHP_SELF]?page=$next&mnu=rabsensi'>Next »</a></span>";
	}
