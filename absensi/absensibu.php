...
<td>:<td><input required name="id_mesin" class="form-control" type="number" id="id_mesin" value="<?php echo $id_mesin;?>" style="width:200px;"   onchange="showUp(this.value)" />
<div id="txtHint"></div>
</td>
<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<?php  
  $sqlc="select distinct(`id_mesin`) from `$tbabsensi` order by `id_mesin` asc";
  $jumc=getJum($conn,$sqlc);
	...
	$arrc=getData($conn,$sqlc);
		foreach($arrc as $dc) {						
				...
				$sql = "select * from `$tbabsensi` where  `id_mesin`='$id_mesin' order by `id_absensi` asc";
		$jum = getJum($conn, $sql);
	?>
